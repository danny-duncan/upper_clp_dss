...
# Find NHD comid for each coordinate. NOTE: resolution not great for some sites 
# (see Comanche/Hourglass complex)... Some samples are along the same flowline, 
# and are therefore represented with the same information. 
distance_upstream_waterbody <- pmap(
  list(comid = xy$comid, 
       geom = xy$geometry, 
       site_code = xy$site_code),
...
distance_to_major_trib <- function(site) {
  ...
  one_site <- xy %>% filter(site_code == site)
