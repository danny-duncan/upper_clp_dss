...
```{r}
wide_sensor_file <- list.files(path = here("data", "collated", "sensor"), pattern = "all_sensor_data_wide_.*\\.parquet$", full.names = TRUE)%>%
  tail(1)
...
water_chem <- water_chem%>%
  filter(!is.na(TOC) & DT_sample >= ymd("2023-09-01") & site_code %in% unique(all_sensor_data_wide$site))%>%
  select(site_code, DT_sample, SC, TN, lab_turb, TOC, ChlA, collector)
...
toc_dataset <- ROSS_model_data%>%
  select(-lab_turb, -lab_cond, -TN, -ChlA)

write_csv(ROSS_model_data, here("data", "derived", "matchup_datasets", paste0("parsed_toc_sensor_clp_", Sys.Date(),".csv")))
...
```{r}
virridy_data <- read_csv(here("data", "derived", "matchup_datasets","parsed_data_virridy.csv"), show_col_types = F)%>%
  dplyr::select(-`...1`, -Phosphorus,-Kjeldahl,-hrs_since_last_cleaning)%>%
  mutate(id = as.character(mw_id))%>%
  select(-mw_id)%>%
  mutate(collector = "Virridy")%>%
  apply_fdom_temp_corr(fdom_col = "FDOM",fdom_corr_col = "FDOM_corr_25c", temp_col = "Temp")%>%
  rename(lab_turb = Turbidity,
         lab_cond = Conductivity,
        `FDOM Fluorescence` = FDOM,
         `Chl-a Fluorescence` = Chl_a,
         Turbidity = Sensor_Turb, 
         `Specific Conductivity` = Sensor_Cond,
         Temperature = Temp)
...
write_csv(all_matched_data, here("data", "derived", "matchup_datasets", paste0("parsed_toc_sensor_virridy_clp_", Sys.Date(),".csv")))
...
ggsave(plot = example_ts_grab, here("docs","uclp_dss", "figs", "example_sensor_timeseries_pfal.png"),
       width = 12, height = 8)
