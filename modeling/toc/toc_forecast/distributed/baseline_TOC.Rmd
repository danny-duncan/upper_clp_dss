...
TOC_GAM <- gam(toc_doc ~ te(DOY, distance_upstream_km, 
                            bs = c("cc", "tp"),
                            k = c(10, 5)),
               data = mainstem)
...
Okay, this seems to work pretty well, even if the p value is low. We do, 
however, want to do some thoughtful TVT here to keep our error estimates
honest. First let's look at annual distribution of data.
...
# fit gam
fit_1 <- gam(toc_doc ~ te(DOY, distance_upstream_km, 
                          bs = c("cc", "tp"),
                          k = c(10, 5)), 
             data = train_1, method = "REML") 

# check gam diagnostics
gam.check(fit_1)

# edf is low relative to k', k index is nearly 1, let it slide. the data are 
# noisy, so it makes sense that we're not optimizing the wiggles
...
# fit gam
fit_2 <- gam(toc_doc ~ te(DOY, distance_upstream_km, 
                          bs = c("cc", "tp"),
                          k = c(10, 5)), 
             data = train_2, method = "REML") 
...
2014 and 2019 are generally under estimated across all sites.
...
# fit gam
fit_3 <- gam(toc_doc ~ te(DOY, distance_upstream_km, 
                          bs = c("cc", "tp"),
                          k = c(10, 5)), 
             data = train_3, method = "REML") 

# check gam diagnostics
gam.check(fit_3) 

# good to go , but a LOT of noise at low TOC
...
# fit gam
fit_4 <- gam(toc_doc ~ te(DOY, distance_upstream_km, 
                          bs = c("cc", "tp"),
                          k = c(10, 5)), 
             data = train_4, method = "REML") 
# check gam diagnostics
gam.check(fit_4)  
...
Some over prediction in 2024, some under prediction in 2016
...
mae_val  <- mae(test$toc_doc, test$pred_avg)
rmse_val <- rmse(test$toc_doc, test$pred_avg)
bias_val <- bias(test$toc_doc, test$pred_avg)

# r2 from linear regression
r2_val <- summary(lm(test$toc_doc ~ test$pred_avg))$r.squared
...
```{r}
ggplot(test, aes(y = pred_avg, x = toc_doc, color = factor(year))) +
  geom_point(alpha = 0.6, size = 2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Observed TOC (mg/L)",
       y = "Predicted TOC (ensemble, mg/L)",
...
```{r, fig.dim=c(12, 8)}
ggplot(test, aes(y = pred_avg, x = toc_doc, color = DOY)) +
  geom_point(alpha = 0.6, size = 2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Observed TOC (mg/L)",
       y = "Predicted TOC (ensemble, mg/L)",
...
# write_rds(fit_1, here("data/derived/models/TOC_distributed/TOC_GAM_fit1_v2026-02-03.rds"))
# write_rds(fit_2, here("data/derived/models/TOC_distributed/TOC_GAM_fit2_v2026-02-03.rds"))
# write_rds(fit_3, here("data/derived/models/TOC_distributed/TOC_GAM_fit3_v2026-02-03.rds"))
# write_rds(fit_4, here("data/derived/models/TOC_distributed/TOC_GAM_fit4_v2026-02-03.rds"))
