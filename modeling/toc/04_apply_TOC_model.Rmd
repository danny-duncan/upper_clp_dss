...
#TODO: this needs to be updated to pull from whatever "complete" dataset the model was trained on
#scaling_params <- readRDS(here("data", "derived", "matchup_dataset", "scaling_parameters_20250820.rds"))
...
#update to correct file path/use a new load_sensor_data_function

prepped_data <- read_parquet(here("data", "collated", "sensor", "all_sensor_data_2023-03-30_2025-08-27.parquet"))%>%
  select(site, 
         DT_round,
...
```{r}
most_recent_file <- list.files("data", "derived","models", "TOC_realtime", "working", pattern = "toc_xgboost_models_light_.*\\.rds", full.names = TRUE) %>%
  tail(1)
