...
ggplot(heat_data, aes(x = factor(TOC_cat_range), y = group, fill = prop)) +
  ...
  labs(
    ...
    # subtitle = paste0(
    #   "Training TOC Range: ", round(min(train_val$TOC, na.rm = TRUE), 2), " - ",
    #   round(max(train_val$TOC, na.rm = TRUE), 2), " mg/L (Median: ", round(median(train_val$TOC), 2), ")\n",
    #   "Testing TOC Range: ", round(min(testing$TOC, na.rm = TRUE), 2), " - ",
    #   round(max(testing$TOC, na.rm = TRUE), 2), " mg/L (Median: ", round(median(testing$TOC), 2), ")"
    # ),
    x = "TOC (mg/L) Category",
    y = "Model Group",
    fill = "Proportion"
...
Keep in mind that this is weighted mlogloss so it may be higher than unweighted mlogloss you see in other places.
